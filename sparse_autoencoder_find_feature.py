...
for i in range(len(freqs)):
    feature_id = i # @param {type:"number"}
    batch_size = 128 # @param {type:"number"}
    # print(i)
    if (large_freq<freqs[i]):
        large_feature_id = feature_id
        large_freq = freqs[i]
        print(f"Feature freq: {freqs[i].item():.4f}")
feature_id  = large_feature_id
import pdb
pdb.set_trace()
