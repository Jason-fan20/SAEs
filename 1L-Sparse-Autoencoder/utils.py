...
if loading_data_first_time:
    ...
    # import pdb
    # pdb.set_trace()
    new_tokens = all_tokens[0:256].clone()
    del all_tokens
...
# def load_partial_data(file_path, start_idx=0, end_idx=None):
#     # Open the file in read mode
#     with open(file_path, 'rb') as f:
#         # Get the storage object
#         storage = torch.storage.UntypedStorage.from_file(f)
        
#         # Create a tensor from the storage
#         tensor = torch.tensor([], dtype=torch.long)
#         tensor.set_(storage)
        
#         # Slice the tensor to get the desired portion
#         return tensor[start_idx:end_idx]

# # Load a portion of the data
# start_idx = 0  # Starting index
# end_idx = 256  # Ending index (adjust as needed)

# all_tokens = load_partial_data("/workspace/data/c4_code_2b_tokens_reshaped.pt", start_idx, end_idx)

# # Shuffle the loaded portion if needed
# all_tokens = shuffle_data(all_tokens)
...
class Buffer():
    ...
    def __init__(self, cfg):
        ...
        self.first = loading_data_first_time
        self.refresh()
